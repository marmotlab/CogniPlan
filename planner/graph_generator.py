...
class Graph_generator:
    def __init__(self, map_size, k_size, sensor_range, target_position, plot=False):
        ...
        self.pred_graph = Graph()
        self.pred_node_coords = None
        self.pred_node_utility = None
        self.pred_direction_vector = None
        self.pred_indicator = None
        self.pred_signal = None
        self.pred_prob = None
    ...
    def update_predict_graph(self, robot_location, pred_max_belief, pred_mean_belief, frontiers):
        self.pred_graph = Graph()
        self.pred_node_coords = self.generate_node_coords(robot_location, pred_max_belief)

        self.find_k_neighbor_all_nodes(self.pred_node_coords, pred_max_belief, prediction=True)
        self.pred_node_utility = []
        self.pred_direction_vector = []
        self.pred_signal = []
        self.pred_prob = []
        node_coords_to_check = self.node_coords[:, 0] + self.node_coords[:, 1] * 1j
        for coords in self.pred_node_coords:
            node = Node(coords, frontiers, pred_max_belief, self.target_position)
            if coords[0] + coords[1] * 1j in node_coords_to_check:
                self.pred_node_utility.append(node.utility)
                self.pred_signal.append(1)
                # assert pred_max_belief[coords[1], coords[0]] == 255, f'{coords} {pred_max_belief[coords[1], coords[0]]}'
            else:
                self.pred_node_utility.append(-50)
                self.pred_signal.append(0)
            self.pred_direction_vector.append(node.direction_vector)
            prob = pred_mean_belief[coords[1], coords[0]]
            self.pred_prob.append(prob)

        self.pred_direction_vector = np.array(self.pred_direction_vector)
        self.pred_node_utility = np.array(self.pred_node_utility)
        self.pred_prob = np.array(self.pred_prob)
        self.pred_signal = np.array(self.pred_signal)
        self.pred_indicator = np.zeros_like(self.pred_signal)
        x = self.pred_node_coords[:,0] + self.pred_node_coords[:,1]*1j
        for node in self.route_node:
            index = np.argwhere(x.reshape(-1) == node[0]+node[1]*1j)[0]
            self.pred_indicator[index] = 1
        return self.pred_node_coords, self.pred_graph.edges, self.pred_node_utility, self.pred_indicator, self.pred_direction_vector, self.pred_prob, self.pred_signal
    ...
    def find_k_neighbor_all_nodes(self, node_coords, robot_belief, ground_truth=False, prediction=False):
        X = node_coords
            ...
            for j, neighbour in enumerate(X[indices[i][:]]):
                ...
                if not self.check_collision(start, end, robot_belief):
                    ...
                    if ground_truth:
                        ...
                        self.pred_graph.add_node(a)
                        self.pred_graph.add_edge(a, b, distances[i, j])
                    else:
                        self.graph.add_node(a)
