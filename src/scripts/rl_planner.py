...
class Runner:
    def __init__(self):
        ...
        parameter.N_GEN_SAMPLE = rospy.get_param('~num_gen_sample', parameter.N_GEN_SAMPLE)
        ...
        self.updatebox_pub = rospy.Publisher('/update_box', Marker, queue_size=1)
    ...
    def init_agent(self):
        ...
        config_file = os.path.join(model_folder, self.config_file)
        generator_file = os.path.join(model_folder, self.generator_file)
        with open(config_file, 'r') as stream:
            config = yaml.load(stream, Loader=yaml.SafeLoader)
        generator = Generator(config['netG'], False)
        generator.load_state_dict(torch.load(generator_file, map_location=self.device, weights_only=True))
    ...
    def run(self, event=None):
        ...
        # planning one more step if next node's utility is zero (sometimes next location is None)
        if self.robot.node_manager.nodes_dict.find(next_location.tolist()) is not None:
            ...
            rospy.logwarn("next location not in node dict")
        ...
        self.run_time_pub.publish(run_time)
        self.waypoint_pub.publish(waypoint_msg)
        grid_msg = self.pub_predicted_map()  # predicted map
        self.pred_map_pub.publish(grid_msg)

        # self.robot.plot_env(self.step, None)
    ...
    def visualize_graph(self):
        ...
        # visualize update box
        updatebox = Marker()
        updatebox.header.frame_id = 'map'
        updatebox.header.stamp = rospy.Time.now()
        ...
        corners = [
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] - half_size, z=0),
            Point(x=self.robot_location[0] + half_size, y=self.robot_location[1] - half_size, z=0),
            Point(x=self.robot_location[0] + half_size, y=self.robot_location[1] + half_size, z=0),
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] + half_size, z=0),
            Point(x=self.robot_location[0] - half_size, y=self.robot_location[1] - half_size, z=0)
